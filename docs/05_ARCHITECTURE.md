...
| `data_safety.py` | SQLite 完整性检查、外键检查和一致性备份 | 不包含业务规则，也不执行自动恢复 |
...
- `schema_migrations` 记录已执行版本；已有数据库升级前先检查并自动备份。
- 新数据库才写初始化数据，已有数据库只执行未应用的 Migration。

## 6. 当前架构债务

1. Goals、Review、Books 的部分写操作绕过 service，规则容易散落在 UI。
2. Schema、迁移、种子数据集中在一个文件，继续增长会难以维护。
3. 已有版本化迁移和迁移前备份，但尚无一键恢复与独立迁移文件目录。
